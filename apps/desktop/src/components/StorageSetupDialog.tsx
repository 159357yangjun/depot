...
export function StorageSetupDialog({
  ...
  return (
    <div
      className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/25 p-6 backdrop-blur-sm"
      onMouseDown={onClose}
    >
      <section
        onMouseDown={(event) => event.stopPropagation()}
        className="max-h-[90vh] w-full max-w-[680px] overflow-auto rounded-[28px] border border-white bg-white p-6 shadow-[0_30px_100px_rgba(15,23,42,.22)]"
      >
        ...
        {isRepository && (
          <div className="mt-6 grid grid-cols-2 gap-4">
            ...
            <Field label="访问令牌" type="password" value={repoForm.token} onChange={(value) => setRepo('token', value)} />
        ...
        {isObject && (
          <div className="mt-6 grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2"><Field label="Endpoint" value={objectForm.endpoint} onChange={(value) => setObject('endpoint', value)} placeholder={defaultObjectEndpoint(provider)} /></div>
        ...
        {isWebDav && (
          <div className="mt-6 grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2"><Field label="WebDAV Endpoint" value={webdavForm.endpoint} onChange={(value) => setWebDav('endpoint', value)} placeholder="https://dav.example.com/remote.php/dav/files/user" /></div>
